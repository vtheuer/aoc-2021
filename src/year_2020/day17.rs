...
fn neighbors_3d(&(x, y, z): &(i8, i8, i8)) -> impl Iterator<Item = (i8, i8, i8)> {
    adjacent(x).flat_map(move |nx| adjacent(y).flat_map(move |ny| adjacent(z).map(move |nz| (nx, ny, nz))))
}
        ...
        space = space
            ...
            .filter(|&cell| {
                let active_neighbors = neighbors(&cell)
                    ...
                    .take(4)
                    .count();
                active_neighbors == 3 || space.contains(&cell) && active_neighbors == 2
            })
            .collect();
    }
...
impl Day<'_> for Day17 {
    ...
    fn part_1(&self) -> Self::T1 {
        run(self.space.iter().map(|&(x, y)| (x, y, 0)).collect(), neighbors_3d)
    }

    fn part_2(&self) -> Self::T2 {
        run(self.space.iter().map(|&(x, y)| (x, y, 0, 0)).collect(), neighbors_4d)
    }
}
